...
- A user collection exposes “Download Collection”. The app loads the complete collection rather than only the visible/search-filtered page, confirms the item count, then creates one FIFO `--no-playlist` job per media item using the current format, quality, and destination.
