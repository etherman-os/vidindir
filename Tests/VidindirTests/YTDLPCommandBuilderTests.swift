...
struct YTDLPCommandBuilderTests {
    ...
    @Test func buildsSafeMP4ArgumentsWithoutShellQuoting() throws {
        ...
        let selector = try #require(value(after: "--format", in: invocation.arguments))
        #expect(selector.contains("vcodec^=avc1"))
        #expect(selector.contains("acodec^=mp4a"))
        #expect(!selector.contains("/bv*+ba/b"))
